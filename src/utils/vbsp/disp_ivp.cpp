...
void Disp_AddCollisionModels( CUtlVector<CPhysCollisionEntry *> &collisionList, dmodel_t *pModel, int contentsMask)
{
	...
	for ( int grid = 0; grid < gDispGridList.Count(); grid++ )
	{
		...
		for ( int listIndex = 0; listIndex < gDispGridList[grid].dispList.Count(); listIndex++ )
		{
			...
			{
				float flAlphaTotal[3] = { 0.0f, 0.0f, 0.0f };
				for ( int iTriVert = 0; iTriVert < 3; ++iTriVert )
				{
					unsigned short index = indices[iTri * 3 + iTriVert];
					pDispInfo->GetVert( index, tmpVerts[iTriVert] );
					if ( ( pDispInfo->GetFlags() & DISP_INFO_FLAG_HAS_MULTIBLEND ) != 0 )
					{
						Vector4D vBlend;
						pDispInfo->GetMultiBlend( index, vBlend );
						flAlphaTotal[0] += vBlend.y;
						flAlphaTotal[1] += vBlend.z;
						flAlphaTotal[2] += vBlend.w;
					}
					else
					{
						flAlphaTotal[0] += pDispInfo->GetAlpha( index );
					}
				}
				...
				if ( ( pDispInfo->GetFlags() & DISP_INFO_FLAG_HAS_MULTIBLEND ) != 0 )
				{
					if ( ( flAlphaTotal[2] > DISP_MULTIBLEND_PROP_THRESHOLD ) && ( flAlphaTotal[2] >= flAlphaTotal[1] ) && ( flAlphaTotal[2] >= flAlphaTotal[0] ) )
					{
						int nProp4 = GetSurfaceProperties2( matID, "surfaceprop4" );
						if ( nProp4 != -1 )
						{
							nProp = nProp4;
						}
					}
					else if ( ( flAlphaTotal[1] > DISP_MULTIBLEND_PROP_THRESHOLD ) && ( flAlphaTotal[1] >= flAlphaTotal[0] ) )
					{
						int nProp3 = GetSurfaceProperties2( matID, "surfaceprop3" );
						if ( nProp3 != -1 )
						{
							nProp = nProp3;
						}
					}
					else if ( flAlphaTotal[0] > DISP_MULTIBLEND_PROP_THRESHOLD )
					{
				...
				{
					if ( flAlphaTotal[0] > DISP_ALPHA_PROP_DELTA )
					{
						int nProp2 = GetSurfaceProperties2( matID, "surfaceprop2" );
						if ( nProp2 != -1 )
						{
