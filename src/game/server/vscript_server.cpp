...
{
public:
	CUtlVector<ScriptVariant_t>	m_msgs;
	CUtlVector<int>	m_types;
	CUtlVector<int>	m_bits;
	CRecipientFilter m_filter;
	string_t m_iMessageName;
	HSCRIPT	m_hScriptInstance;

	CScriptUserMessage()
	{
		m_iMessageName = NULL_STRING;
	...
	void WriteData( ScriptVariant_t data, int type, int numbits = -1 )
	{
		m_msgs.AddToTail( data );
		m_types.AddToTail( type );
		m_bits.AddToTail( numbits );
	}
	const char *GetMessageName()
	{
		return STRING( m_iMessageName );
	}
	void WriteInt( int value, int type )
	{
		ScriptVariant_t data( value );
		WriteData( data, type );
	}
	void WriteFloat( float value, int type )
	{
		ScriptVariant_t data( value );
		WriteData( data, type );
	}
	...
	void WriteVector( const Vector &value, int type )
	{
		ScriptVariant_t data( value );
		WriteData( data, type );
	}
	...
	{
		ScriptVariant_t data( value, true );
		WriteData( data, 13 );
	}
	void WriteEntity( HSCRIPT hEntity )
	{
		ScriptVariant_t data( hEntity );
		WriteData( data, 14 );
	}
	void WriteBitLong( int value, int numbits, bool bSigned )
	{
		ScriptVariant_t data( value );
		int type = ( bSigned ) ? 17 : 16;
		WriteData( data, type, numbits );
	}
	void EndMessage()
	{
		UserMessageBegin( m_filter, GetMessageName() );
		for ( int nEntry = 0; nEntry < m_msgs.Count(); nEntry++ )
		{
			ScriptVariant_t value = m_msgs[nEntry];
			int type = m_types[nEntry];

			switch ( type )
			{
				case 0:
				{
					WRITE_BYTE( value.m_int );
					break;
				}
				case 1:
				{
					WRITE_CHAR( value.m_int );
					break;
				}
				case 2:
				{
					WRITE_SHORT( value.m_int );
					break;
				}
				case 3:
				{
					WRITE_WORD( value.m_int );
					break;
				}
				case 4:
				{
					WRITE_LONG( value.m_int );
					break;
				}
				case 5:
				{
					if ( value.m_type == FIELD_FLOAT )
						WRITE_FLOAT( value.m_float );
					else
						WRITE_FLOAT( (float)value.m_int );
					break;
				}
				case 6:
				{
					if ( value.m_type == FIELD_FLOAT )
						WRITE_ANGLE( value.m_float );
					else
						WRITE_ANGLE( (float)value.m_int );
					break;
				}
				case 7:
				{
					if ( value.m_type == FIELD_FLOAT )
						WRITE_COORD( value.m_float );
					else
						WRITE_COORD( (float)value.m_int );
					break;
				}
				case 8:
				{
					WRITE_BOOL( value.m_bool );
					break;
				}
				case 9:
				{
					WRITE_VEC3COORD( *value.m_pVector );
					break;
				}
				case 10:
				{
					WRITE_VEC3NORMAL( *value.m_pVector );
					break;
				}
				case 11:
				{
					WRITE_VEC3_INTEGRAL( *value.m_pVector );
					break;
				}
				case 12:
				{
					QAngle angles( VectorExpand( *value.m_pVector ) );
					WRITE_ANGLES( angles );
					break;
				}
				case 13:
				{
					WRITE_STRING( value.m_pszString );
					break;
				}
				case 14:
				{
					CBaseEntity *pEntity = ToEnt( value.m_hScript );
					WRITE_EHANDLE( pEntity );
					break;
				}
				case 15:
				{
					WRITE_ENTITY( value.m_int );
					break;
				}
				case 16:
				{
					WRITE_UBITLONG( (unsigned int)value.m_int, m_bits[nEntry] );
					break;
				}
				case 17:
				{
					WRITE_SBITLONG( value.m_int, m_bits[nEntry] );
					break;
				}
			}
		}
...
class CScriptUserMessageScriptInstanceHelper : public IScriptInstanceHelper
{
	bool ToString( void *p, char *pBuf, int bufSize )
	{
		CScriptUserMessage *pUM = (CScriptUserMessage *)p;
		V_snprintf( pBuf, bufSize, "(UserMessage: %s)", pUM->GetMessageName() );
		return true;
	}
};

CScriptUserMessageScriptInstanceHelper g_ScriptUserMessageScriptInstanceHelper;

BEGIN_SCRIPTDESC_ROOT( CScriptUserMessage, "ScriptUserMessage class" )
	DEFINE_SCRIPT_INSTANCE_HELPER( &g_ScriptUserMessageScriptInstanceHelper )
	DEFINE_SCRIPTFUNC( GetMessageName, "Returns the name of this usermessage" );
	DEFINE_SCRIPTFUNC( Delete, "Deletes this usermessage instance" );
	DEFINE_SCRIPTFUNC( ClearData, "Clear all stored written data" );
	DEFINE_SCRIPTFUNC( WriteInt, "Arguments: ( value, type ) - Store integer data for write operation" );
	DEFINE_SCRIPTFUNC( WriteFloat, "Arguments: ( value, type ) - Store float data for write operation" );
	DEFINE_SCRIPTFUNC( WriteBool, "Store boolean data for write operation" );
	DEFINE_SCRIPTFUNC( WriteVector, "Arguments: ( value, type ) - Store Vector data for write operation" );
	DEFINE_SCRIPTFUNC( WriteString, "Store string data for write operation" );
	DEFINE_SCRIPTFUNC( WriteEntity, "Store entity data for write operation" );
	DEFINE_SCRIPTFUNC( WriteBitLong, "Arguments: ( value, numBits, bSigned ) - Store bit long data for write operation" );
	DEFINE_SCRIPTFUNC( EndMessage, "Finish the usermessage and dispatch it to clients" );
END_SCRIPTDESC();

class CScriptUserMessages
{
public:
	HSCRIPT Begin( HSCRIPT hPlayer, const char *messageName )
	{
		int msg_type = usermessages->LookupUserMessage( messageName );
		if ( msg_type == -1 )
		{
			DevMsg( "Error: Unregistered message \"%s\" in UserMessages.Begin\n", messageName );
			return NULL;
		}
		...
		{
			CReliableBroadcastRecipientFilter broadcast;
			filter = broadcast;
		}

		CScriptUserMessage *pUM = new CScriptUserMessage;
		pUM->m_iMessageName = AllocPooledString( messageName );
		pUM->m_filter = filter;
		return pUM->GetScriptInstance();
	}
	...
	{
		if ( !usermessages->IsValidIndex( index ) )
			return -1;

		return usermessages->GetUserMessageSize( index );
	}
	const char *GetUserMessageName( int index )
	{
		...
		return usermessages->GetUserMessageName( index );
	}
	bool IsValidIndex( int index )
	{
		return usermessages->IsValidIndex( index );
	}
	int Count()
	{
		int idx = 0;
		while ( usermessages->IsValidIndex( idx ) )
		{
			idx++;
		}
		return idx;
	}
} g_ScriptUserMessages;

BEGIN_SCRIPTDESC_ROOT( CScriptUserMessages, SCRIPT_SINGLETON "Used to send user messages to clients" )
	DEFINE_SCRIPTFUNC( LookupUserMessage, "Arguments: ( messageName ) - Returns -1 if not found, otherwise, returns appropriate index" )
	DEFINE_SCRIPTFUNC( GetUserMessageSize, "Arguments: ( index ) - Returns the size of the usermessage" )
	DEFINE_SCRIPTFUNC( GetUserMessageName, "Arguments: ( index ) - Returns the name of the usermessage" )
	DEFINE_SCRIPTFUNC( IsValidIndex, "Arguments: ( index ) - Returns true if the usermessage index is valid" )
	DEFINE_SCRIPTFUNC( Count, "Returns number of registered user messages." )
	DEFINE_SCRIPTFUNC( Begin, "Arguments: ( player, messageName ) - Returns a new usermessage instance" )
END_SCRIPTDESC();
	...
	if( scriptmanager != NULL )
	{
		...
		if( scriptLanguage != SL_NONE )
		{
			...
			if( g_pScriptVM )
			{
				...
				g_pScriptVM->RegisterInstance( &g_ScriptUserMessages, "UserMessages" );
				...
				// To be used with CScriptUserMessage Write functions
				g_pScriptVM->SetValue( "UM_BYTE", 0 );
				g_pScriptVM->SetValue( "UM_CHAR", 1 );
				g_pScriptVM->SetValue( "UM_SHORT", 2 );
				g_pScriptVM->SetValue( "UM_WORD", 3 );
				g_pScriptVM->SetValue( "UM_LONG", 4 );
				g_pScriptVM->SetValue( "UM_FLOAT", 5 );
				g_pScriptVM->SetValue( "UM_ANGLE", 6 );
				g_pScriptVM->SetValue( "UM_COORD", 7 );
				g_pScriptVM->SetValue( "UM_BOOL", 8 );
				g_pScriptVM->SetValue( "UM_VEC3COORD", 9 );
				g_pScriptVM->SetValue( "UM_VEC3NORMAL", 10 );
				g_pScriptVM->SetValue( "UM_VEC3_INTEGRAL", 11 );
				g_pScriptVM->SetValue( "UM_ANGLES", 12 );
				g_pScriptVM->SetValue( "UM_STRING", 13 );
				g_pScriptVM->SetValue( "UM_EHANDLE", 14 );
				g_pScriptVM->SetValue( "UM_ENTITY", 15 );
				g_pScriptVM->SetValue( "UM_UBITLONG", 16 );
				g_pScriptVM->SetValue( "UM_SBITLONG", 17 );
