...
class CASW_Filter_Incapacitated : public CBaseFilter
{
	DECLARE_CLASS( CASW_Filter_Incapacitated, CBaseFilter );
...
public:
	bool PassesFilterImpl( CBaseEntity *pCaller, CBaseEntity *pEntity ) override
	{
		CASW_Marine *pMarine = CASW_Marine::AsMarine( pEntity );
		if ( pMarine )
			return pMarine->m_bKnockedOut;

#ifdef RD_7A_WEAPONS
		if ( pEntity && pEntity->Classify() == CLASS_ASW_REVIVE_TOOL_MARKER )
			return true;
#endif
...
LINK_ENTITY_TO_CLASS( asw_filter_incapacitated, CASW_Filter_Incapacitated );
