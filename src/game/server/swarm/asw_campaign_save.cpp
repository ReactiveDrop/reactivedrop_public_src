...
ConVar asw_custom_skill_points( "asw_custom_skill_points", "0", FCVAR_CHEAT, "If set, marines will start with no skill points and will spend them as they progress through the campaign." );
...
void CASW_Campaign_Save::SetMoveDestination( int iMission )
{
	...
	const RD_Campaign_t *pCI = ASWGameRules()->GetCampaignInfo();
	if ( !pCI )
		return;

	const RD_Campaign_Mission_t *pMission = pCI->GetMission( iMission );
	if ( !pMission )
		return;
...
float CASW_Campaign_Save::EstimateCost( const RD_Campaign_t *pCI, int iStart, int iEnd )
{
	// heuristic for A*
	const RD_Campaign_Mission_t *pMission = pCI->GetMission( iStart );
	const RD_Campaign_Mission_t *pEndMission = pCI->GetMission( iEnd );
	if ( !pMission || !pEndMission )
		return 9000;

	float diff_x = pEndMission->LocationX - pMission->LocationX;
	float diff_y = pEndMission->LocationY - pMission->LocationY;
...
bool CASW_Campaign_Save::BuildCampaignRoute( int iStart, int iEnd )
{
	...
	const RD_Campaign_t *pCI = ASWGameRules()->GetCampaignInfo();
	if ( iStart < 0 || iStart >= pCI->Missions.Count() )
		return false;
	if ( iEnd < 0 || iEnd >= pCI->Missions.Count() )
		return false;
	...
	while ( iOverflow < 1000 )
	{
		...
		const RD_Campaign_Mission_t *pMission = pCI->GetMission( iCurrentMission );
		if ( !pMission )
		{
		...
		{
			int iOtherMission = pMission->Links[i];
			//Msg("other mission[%d] is %d\n", i, iOtherMission);
			const RD_Campaign_Mission_t *pOtherMission = pCI->GetMission( pMission->Links[i] );
			if ( !pOtherMission )
				continue;

			// check if it's on the open list
			int iOnOpenList = -1;
			FOR_EACH_VEC( m_OpenList, k )
			{
...
void CASW_Campaign_Save::SelectDefaultNextCampaignMission()
{
	...
	for ( int i = 1; i < pInfo->Missions.Count(); i++ )		// skip first dummy entry
	{
