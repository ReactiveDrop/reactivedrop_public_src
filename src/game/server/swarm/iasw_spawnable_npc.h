...
{
public:
	// inform our spawnable about the spawner, so the spawn can be notified of the spawnable's death
	virtual void SetSpawner( CASW_Base_Spawner *spawner ) = 0;
	virtual CAI_BaseNPC *GetNPC() = 0;
	// spawnables should return true if they can spawn underground
	virtual bool CanStartBurrowed() = 0;
	// should make the spawnable start underground
	virtual void StartBurrowed() = 0;
	...
	virtual void SetHealthByDifficultyLevel() = 0;
	// this alien was spawned as part of a holdout wave
	virtual void SetHoldoutAlien() = 0;
