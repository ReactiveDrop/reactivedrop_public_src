...
BEGIN_DATADESC( CASW_Colonist )
	...
	DEFINE_FIELD( m_hFollowingMarine, FIELD_EHANDLE ),
	DEFINE_KEYFIELD(	m_bAimTarget, FIELD_BOOLEAN, "aimtarget" ),
	DEFINE_KEYFIELD(	m_bFollowOnUse, FIELD_BOOLEAN, "followonuse" ),
	DEFINE_KEYFIELD(	m_bDropWeaponOnDeath, FIELD_BOOLEAN, "dropweaponondeath" ),
...
IMPLEMENT_SERVERCLASS_ST( CASW_Colonist, DT_ASW_Colonist )
	SendPropBool( SENDINFO( m_bAimTarget ) ),
	SendPropBool( SENDINFO( m_bFollowOnUse ) ),
	SendPropEHandle( SENDINFO( m_hFollowingMarine ) ),
END_SEND_TABLE()
...
CASW_Colonist::CASW_Colonist()
{
	...
	m_bAimTarget = false;
	m_bFollowOnUse = true;
	m_hFollowingMarine = NULL;
	m_bDropWeaponOnDeath = true;
}
...
bool CASW_Colonist::CreateBehaviors()
{
...
int CASW_Colonist::SelectSchedule()
{
	int nSched = SelectFlinchSchedule_ASW();
	if ( nSched != SCHED_NONE )
		return nSched;

	if ( m_hFollowingMarine.Get() )
...
bool CASW_Colonist::IsSelectedBy( CASW_Marine *pMarine )
{
	return m_hFollowingMarine.Get() == pMarine;
}
