...
// go big or go home
#define ASW_QUEEN_HEALTH_BITS 31

LINK_ENTITY_TO_CLASS( asw_queen, CASW_Queen );

IMPLEMENT_SERVERCLASS_ST( CASW_Queen, DT_ASW_Queen )
	SendPropExclude( "DT_BaseAnimating", "m_flPoseParameter" ),
	SendPropExclude( "DT_ASW_Inhabitable_NPC", "m_iHealth" ), // we need more bits

	SendPropEHandle( SENDINFO( m_hQueenEnemy ) ),
	SendPropBool( SENDINFO( m_bChestOpen ) ),
	SendPropInt( SENDINFO( m_iHealth ), ASW_QUEEN_HEALTH_BITS + 1 ),
	SendPropInt( SENDINFO( m_iMaxHealth ), ASW_QUEEN_HEALTH_BITS, SPROP_UNSIGNED ),
...
BEGIN_DATADESC( CASW_Queen )
	...
	DEFINE_FIELD( m_fLastRangedAttack, FIELD_TIME ),
	DEFINE_FIELD( m_fLastSpawnedParasites, FIELD_TIME ),
	...
	DEFINE_KEYFIELD( m_bCanMove, FIELD_BOOLEAN, "CanMove" ),
	DEFINE_KEYFIELD( m_bCanClaw, FIELD_BOOLEAN, "CanClaw" ),
	DEFINE_KEYFIELD( m_bCanSpit, FIELD_BOOLEAN, "CanSpit" ),
	DEFINE_KEYFIELD( m_bCanDive, FIELD_BOOLEAN, "CanDive" ),
	DEFINE_KEYFIELD( m_bCanSpawnParasites, FIELD_BOOLEAN, "CanSpawnParasites" ),
...
ConVar asw_queen_has_damage_resistances( "asw_queen_has_damage_resistances", "1", FCVAR_CHEAT, "Is the queen resistant to certain damage types" );
ConVar asw_queen_diver_interval( "asw_queen_diver_interval", "20", FCVAR_CHEAT, "Time it takes queen's divers to recharge" );
...
// (these will only fire outputs; it's up to the mapper to actually spawn the aliens)
#define QUEEN_SUMMON_WAVE_POINT_1 0.8f
#define QUEEN_SUMMON_WAVE_POINT_2 0.6f
#define QUEEN_SUMMON_WAVE_POINT_3 0.4f
#define QUEEN_SUMMON_WAVE_POINT_4 0.2f
...
CASW_Queen::CASW_Queen()
{
	...
	m_bCanSpit = true;
	m_bCanDive = true;
	m_bCanSpawnParasites = false;

	m_iDiverState = ASW_QUEEN_DIVER_NONE;
	m_fLastDiverAttack = 0;
	m_fLastRangedAttack = 0;
	m_fLastSpawnedParasites = 0;
...
{
	return m_bCanClaw ? ASW_QUEEN_MELEE_RANGE : 0;
}
...
float CASW_Queen::InnateRange2MinRange( void )
{
	return 0;
}

float CASW_Queen::InnateRange2MaxRange( void )
{
	return ASW_QUEEN_MAX_ATTACK_DISTANCE;
}
...
{
	if ( m_bCanClaw && flDist < ASW_QUEEN_MELEE_RANGE )
	{
		return COND_TOO_CLOSE_TO_ATTACK;
	}
...
int CASW_Queen::RangeAttack2Conditions( float flDot, float flDist )
{
	if ( flDist > ASW_QUEEN_MAX_ATTACK_DISTANCE )
	{
		return COND_TOO_FAR_TO_ATTACK;
	}
	else if ( flDot < 0.5 )
...
void CASW_Queen::GatherConditions()
{
	...
	ClearCondition( COND_QUEEN_BLOCKED_BY_BREAKABLE_OBJECT );
	if ( m_hBlockingSentry.Get() )
	{
		SetCondition( COND_QUEEN_BLOCKED_BY_BREAKABLE_OBJECT );
	}
}
...
int CASW_Queen::SelectQueenCombatSchedule()
{
	...
	if ( m_bCanSpit && GetShotRegulator()->IsInRestInterval() )
	{
	...
	if ( m_bCanDive )
	{
		...
		if ( ( m_fLastDiverAttack == 0 || gpGlobals->curtime > m_fLastDiverAttack + asw_queen_diver_interval.GetFloat() ) )
			return SCHED_START_DIVER_ATTACK;
	}

	// if we're blocked by a sentry, smash that mofo
	if ( HasCondition( COND_QUEEN_BLOCKED_BY_BREAKABLE_OBJECT ) )
	...
	if ( m_bCanSpawnParasites )
	{
		if ( m_fLastSpawnedParasites == 0 )
			m_fLastSpawnedParasites = gpGlobals->curtime;	// forces delay before first parasites
		if ( gpGlobals->curtime > m_fLastSpawnedParasites + asw_queen_parasite_interval.GetFloat() )
		{
			if ( HasCondition( COND_CAN_RANGE_ATTACK2 ) && random->RandomFloat() < 0.5f )
			{
	...
	if ( m_bCanSpit )
	{
		if ( m_fLastRangedAttack == 0 )
			m_fLastRangedAttack = gpGlobals->curtime;	// forces delay before first spit attack
		if ( gpGlobals->curtime > m_fLastRangedAttack + asw_queen_spit_interval.GetFloat() )
		{
			if ( HasCondition( COND_CAN_RANGE_ATTACK1 ) )
			{
	...
	return m_bCanMove ? SCHED_CHASE_ENEMY : SCHED_COMBAT_FACE;
}
...
void CASW_Queen::RunTask( const Task_t *pTask )
{
	...
	case TASK_SPAWN_PARASITES:
	{
		...
		{
			if ( m_iCrittersAlive >= asw_queen_max_parasites.GetInt() || m_iCrittersSpawnedRecently > asw_queen_max_parasites.GetInt() )
			{
				SetChestOpen( false );
				TaskComplete();
			}
...
int CASW_Queen::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	// make queen immune to crush (physics) damage
	if ( info.GetDamageType() & DMG_CRUSH )
	{
		return 0;
	}

	if ( !asw_queen_has_damage_resistances.GetBool() )
...
{
	if ( info.GetAttacker() && info.GetAttacker()->MyCombatCharacterPointer() && info.GetAttacker()->MyCombatCharacterPointer()->IRelationType( this ) == D_LI )
		return false;
...
int	CASW_Queen::DrawDebugTextOverlays()
{
	...
	if ( m_debugOverlays & OVERLAY_TEXT_BIT )
	{
		...
		NDebugOverlay::EntityText( entindex(), text_offset, HasCondition( COND_CAN_RANGE_ATTACK1 ) ? "Can Range Attack1" : "No Range attack1", 0 );
		text_offset++;
		NDebugOverlay::EntityText( entindex(), text_offset, HasCondition( COND_CAN_RANGE_ATTACK2 ) ? "Can Range Attack2" : "No Range attack2", 0 );
		text_offset++;
...
bool CASW_Queen::IsHeavyDamage( const CTakeDamageInfo &info )
{
	...
	if ( ( info.GetDamageType() & DMG_BURN ) != 0 )
	{
		...
		if ( bFlinch && asw_debug_alien_damage.GetBool() )
	...
	AIMoveResult_t *pResult )
{
	...
	{
		// check if we collide with a sentry
...
AI_BEGIN_CUSTOM_NPC( asw_queen, CASW_Queen )
	...
	// conditions
	DECLARE_CONDITION( COND_QUEEN_BLOCKED_BY_BREAKABLE_OBJECT )
