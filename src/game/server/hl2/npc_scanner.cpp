...
#define	SPOTLIGHT_WIDTH					90

#define SCANNER_SPOTLIGHT_NEAR_DIST		64
#define SCANNER_SPOTLIGHT_FAR_DIST		128
...
ConVar	sk_scanner_health( "sk_scanner_health", "30", FCVAR_CHEAT );
...
// reactivedrop: reimplementing CAI_BaseNPC::UpdateSleepState() because
// AISS_WAITING_FOR_INPUT doesn't work correctly
void CNPC_CScanner::UpdateSleepState( bool bInPVS )
{
	if ( GetSleepState() > AISS_AWAKE )
	{
/*		if ( GetWakeRadius() > .1f && ( pLocalPlayer->GetAbsOrigin() - GetAbsOrigin() ).LengthSqr() <= Square(GetWakeRadius()) )
			Wake();
