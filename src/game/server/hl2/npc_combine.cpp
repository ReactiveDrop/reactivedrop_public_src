...
{
	s_iszShotgunClassname = FindPooledString( "asw_weapon_shotgun" );
	BaseClass::Activate();
}
...
void CNPC_Combine::StartTask( const Task_t *pTask )
{
	...
	case TASK_COMBINE_SIGNAL_BEST_SOUND:
		...
		{
			bool bCorpse;
			CASW_Marine *pMarine = UTIL_ASW_AnyMarineCanSee( GetAbsOrigin(), 384.0f, bCorpse );
			if( pMarine && !bCorpse && OccupyStrategySlot( SQUAD_SLOT_EXCLUSIVE_HANDSIGN ) )
			{
	...
	case TASK_FIND_COVER_FROM_ENEMY:
		{
			if (GetHintGroup() == NULL_STRING)
			{
				...
				if ( pEntity )
				{
					...
					if ( !HasMemory(bits_MEMORY_PLAYER_HURT) && pEntity->Classify() == CLASS_ASW_MARINE && pEntity->GetHealth() <= 20 )
					{
						if ( m_pSquad )
						{
...
{
#ifdef INFESTED_DLL
	if ( GetActiveWeapon() && !ShouldDropActiveWeaponWhenKilled() )
	{
...
Activity CNPC_Combine::NPC_TranslateActivity( Activity eNewActivity )
{
	...
	if (eNewActivity == ACT_RANGE_ATTACK2)
	{
		...
		if ( Weapon_OwnsThisType( "asw_weapon_grenade_launcher" ) )
		{
...
void CNPC_Combine::AnnounceEnemyType( CBaseEntity *pEnemy )
{
	...
	switch ( (int)pEnemy->Classify() )
	{
	case CLASS_PLAYER:
	case CLASS_ASW_MARINE:
...
void CNPC_Combine::AnnounceEnemyKill( CBaseEntity *pEnemy )
{
	...
	if ( pEnemy->Classify() == CLASS_ASW_MARINE )
			...
			if ( HasCondition(COND_HEAR_DANGER) )
			{
				...
				if ( pSound)
				{
					if (pSound->m_iType & SOUND_DANGER)
					{
						...
						if ( pSoundOwner )
						{
							CASW_Grenade_Vindicator *pGrenade = dynamic_cast< CASW_Grenade_Vindicator * >( pSoundOwner );
							if ( pGrenade && pGrenade->m_hFirer )
							{
								if ( IRelationType( pGrenade->m_hFirer ) != D_LI )
								{
...
int CNPC_Combine::SelectScheduleAttack()
{
	...
	if ( GetEnemy() && GetEnemy()->Classify() == CLASS_ASW_SENTRY_BASE )
	{
	...
	case SCHED_TAKE_COVER_FROM_ENEMY:
		{
			if ( m_pSquad )
			{
				...
				if ( ASWGameRules() && ASWGameRules()->m_iSkillLevel >= 3 &&
...
#define MIN_SIGNAL_DIST	256
			if ( GetEnemy() != NULL && GetEnemy()->Classify() == CLASS_ASW_MARINE && m_bFirstEncounter )
			{
		...
		case COMBINE_AE_KICK:
			{
				...
				if (pBCC)
				{
					...
					{
						if ( pBCC->Classify() == CLASS_ASW_MARINE )
						{
...
{
	if( FClassnameIs( pWeapon, "asw_weapon_ar2" ) )
	{
	...
	else if( FClassnameIs( pWeapon, "asw_weapon_shotgun" )	)
	{
	...
	else if( FClassnameIs( pWeapon, "asw_weapon_pdw" ) )
	{
		return WEAPON_PROFICIENCY_GOOD;
	}
