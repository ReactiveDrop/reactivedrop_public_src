...
//Melee definitions
#define	ANTLIONGUARD_MELEE1_RANGE		100.0f
...
//-----------------------------------------------------------------------------
// Purpose: 
// reactivedrop: TODO investigate, may be it was more feasible to inherit from 
// CASW_Alien then from CASW_Alien_Shover
//-----------------------------------------------------------------------------
class CNPC_AntlionGuard : public CASW_Alien_Shover
{
public:
	DECLARE_CLASS( CNPC_AntlionGuard, CASW_Alien_Shover );
	DECLARE_SERVERCLASS();
	DECLARE_DATADESC();
	...
	enum
	{
		COND_ANTLIONGUARD_PHYSICS_TARGET = BaseClass::NEXT_CONDITION,
	...
	{
		extern ConVar asw_shieldbug_speedboost;
...
CNPC_AntlionGuard::CNPC_AntlionGuard( void )
{
	...
	// reactivedrop: we set this to true for now to spawn it by spawn manager 
	//m_bInCavern = true;

	// reactivedrop: just for testing 
	if (rd_antlionguard_incavern.GetInt())
	{
		m_bInCavern = true;
		m_bCavernBreed = true;
	}
...
LINK_ENTITY_TO_CLASS( npc_antlionguard_normal, CNPC_AntlionGuard );
LINK_ENTITY_TO_CLASS( npc_antlionguard_cavern, CNPC_AntlionGuard );
...
void CNPC_AntlionGuard::Precache( void )
{
	...
	PrecacheModel( "models/gib_antlionguard_hand1_p1.mdl" );
	PrecacheModel( "models/gib_antlionguard_hand1_p2.mdl" );
	PrecacheModel( "models/gib_antlionguard_hand2_p1.mdl" );
	PrecacheModel( "models/gib_antlionguard_hand2_p2.mdl" );
	PrecacheModel( "models/gib_antlionguard_leg1_p1.mdl" );
...
#endif // HL2_EPISODIC
	// reactivedrop: moved from HL2_EPISODIC
	...
	PrecacheParticleSystem ( "mortarbug_death" );
...
void CNPC_AntlionGuard::Spawn( void )
{
	...
	else if ( ClassMatches( "npc_antlionguard_normal" ) )
	{
		m_bCavernBreed = false;
		m_bInCavern = false;
	}
...
int CNPC_AntlionGuard::SelectCombatSchedule( void )
{
	...
	*/
	...
	// reactivedrop: because AlienShover::SelectSchedule causes bugs 
...
bool CNPC_AntlionGuard::ShouldCharge( const Vector &startPos, const Vector &endPos, bool useTime, bool bCheckForCancel )
{
	...
	// reactivedrop: don't charge if electro stunned 
...
float CNPC_AntlionGuard::MaxYawSpeed( void )
{
	...
	// reactivedrop: guard turns slower when running at marine 
	if ( pEnemy != NULL && !CASW_Marine::AsMarine( pEnemy ))
		return 16.0f;
	// reactivedrop: guard turns slower when stunned by tesla 
	if (m_bElectroStunned.Get())
		return 10.0f;
...
void CNPC_AntlionGuard::Shove( void )
{
	...
	{
		pTarget->TakeDamage( CTakeDamageInfo( this, this, 1.0f, DMG_SLASH ) );
		m_hShoveTarget = NULL;
		return;
	}
	...
	{
		float length = (vecEnd - WorldSpaceCenter()).Length();
		Vector direction = (vecEnd - WorldSpaceCenter());
		VectorNormalize(direction);
		Vector hullMaxs = Vector(16, 16, 16);
		hullMaxs.x = length + hullMaxs.x;
		NDebugOverlay::BoxDirection(WorldSpaceCenter(), Vector(-16,-16,-16), hullMaxs, direction, 100, 255, 255, 20, 1.0);
		NDebugOverlay::BoxDirection(WorldSpaceCenter(), Vector(-16,-16,-16),  Vector(16,16,16), direction, 255, 0, 0, 20, 1.0);
	}

	// Use the melee trace to ensure we hit everything there
	trace_t tr;
	CTakeDamageInfo	dmgInfo( this, this, damage, DMG_SLASH );
	CASW_Trace_Filter_Melee traceFilter( this, COLLISION_GROUP_NONE, this, false );
	Ray_t ray;
...
public:
	...
	virtual bool ShouldHitEntity( IHandleEntity *pHandleEntity, int contentsMask )
	{
		...
		if ( pEntity )
		{
			...
			CTakeDamageInfo info( m_pAttacker, m_pAttacker, flDamage, DMG_SLASH );
...
// 		return;
	...
	// reactivedrop: hardcoded 3600, TODO: replace with local player dist
	//float flDistanceToPlayerSqr = ( pPlayer->GetAbsOrigin() - GetAbsOrigin() ).LengthSqr();
	float flDistanceToPlayerSqr = 3600;
...
void CNPC_AntlionGuard::GetPhysicsShoveDir( CBaseEntity *pObject, float flMass, Vector *pOut )
{
	...
	if ( vecToss == vec3_origin )
	{
		...
		if ( vecToss == vec3_origin )
		{
			const float flGravity = GetGravity();
...
void CNPC_AntlionGuard::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->Event() == AE_ANTLIONGUARD_SHOVE_PHYSOBJECT )
	{
		...
		if ( pEnemy != NULL )
		{
			...
			if ( /*NPC_Rollermine_IsRollermine( m_hPhysicsTarget )*/false )
			{
...
int CNPC_AntlionGuard::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if ( g_pGameRules->GetSkillLevel() >= 3 && !(info.GetDamageType() & DMG_CRUSH) )
	{
	...
	if ( IsHeavyDamage( dInfo ) )
	{
		...
		if ( info.GetInflictor() && /*UTIL_IsCombineBall( info.GetInflictor() )*/false )
		{
			dInfo.SetDamage( 50 );
		}
	...
	// reactivedrop: prevent slow movement if not electro stunned 
	// because even bullets make it walk really slow 
	if ( !m_bElectroStunned.Get() && ShouldMoveSlow() )
		m_fHurtSlowMoveTime = gpGlobals->curtime - 0.1; 
...
//	DoBloodDecal( info.GetDamage(), ptr->endpos, vecDir, ptr, info.GetDamageType() );
...
void CNPC_AntlionGuard::PrescheduleThink( void )
{
	...
	// reactivedrop: don't do thinking when frozen 
	if ( IsMovementFrozen() )
		return;
...
// 	}
	...
	if (pMarine)
	{
		Vector vecForceDir = (pTarget->WorldSpaceCenter() - WorldSpaceCenter());
		vecForceDir.NormalizeInPlace();
		extern ConVar asw_shieldbug_knockdown_force;
		extern ConVar asw_shieldbug_knockdown_lift;
		vecForceDir *= asw_shieldbug_knockdown_force.GetFloat();
	...
	float flDamage = sk_antlionguard_dmg_charge.GetFloat();
...
void CNPC_AntlionGuard::Event_Killed( const CTakeDamageInfo &info )
{
	...
	// reactivedrop: always gib unless frozen or on fire
	if ( m_flFrozen < 0.1f && !m_bOnFire )
	{ 
		m_nDeathStyle = kDIE_INSTAGIB;

		// put a green blood decal underneath
		trace_t tr;
...
{
	// reactivedrop: we don't want die animation 
	return true;
...
bool CNPC_AntlionGuard::IsHeavyDamage( const CTakeDamageInfo &info )
{
	...
	// reactivedrop: we want tesla to stop charging
	// commented for now, this causes antlion guard to
	// quickly attack(and kill) cornered player who is under ECA effect
	// taking SHOCK damage is considered as heavy damage and interrupts
	// the current attack and immediately starts the new one
