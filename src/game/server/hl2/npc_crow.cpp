...
void CNPC_Crow::GatherEnemyConditions( CBaseEntity *pEnemy )
{
	...
	// BenLubar: halved distances to better suit Alien Swarm's top-down camera.
...
bool CNPC_Crow::Probe( const Vector &vecMoveDir, float flSpeed, Vector &vecDeflect )
{
	...
	//
	trace_t tr;
	AI_TraceHull( GetAbsOrigin(), GetAbsOrigin() + vecMoveDir * flSpeed, GetHullMins(), GetHullMaxs(), MASK_NPCSOLID, this, ASW_COLLISION_GROUP_BUZZER, &tr );
	if ( tr.fraction < 1.0f )
	{
		//
...
//		}
		...
		case TASK_CROW_FLY:
		{
			...
			Vector vecNewVelocity( cosf( DEG2RAD( flYaw ) ), sinf( DEG2RAD( flYaw ) ), random->RandomFloat( 0.1f, 0.5f ) );
	...
	switch ( pTask->iTask )
	{
		...
		case TASK_CROW_HOP:
		{
			...
			if ( ( GetAbsOrigin().z < m_flHopStartZ ) && ( !( GetFlags() & FL_ONGROUND ) ) )
			{
				...
				//
				trace_t tr;
				AI_TraceLine( GetAbsOrigin(), GetAbsOrigin() + Vector( 0, 0, -32 ), MASK_SOLID, this, ASW_COLLISION_GROUP_BUZZER, &tr );
				if ( tr.fraction == 1.0f )
				{
					//
...
int CNPC_Crow::SelectSchedule( void )
{
	...
	if ( HasCondition( COND_CROW_ENEMY_TOO_CLOSE ) && m_flDangerSoundTime <= gpGlobals->curtime )
	{
		...
		// BenLubar: halved distances to better suit Alien Swarm's top-down camera.
		if ( m_flEnemyDist > 200 )
		{
			return SCHED_CROW_WALK_AWAY;
		}
		else if ( m_flEnemyDist > 150 )
		{
			m_nMorale -= 1;
