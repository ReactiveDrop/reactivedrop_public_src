...
// Animation events.
//-----------------------------------------------------------------------------
// reactivedrop: made static because linking error with asw_parasite, TODO: investigate 
static int AE_HEADCRAB_JUMPATTACK;
...
enum
{
	SCHED_HEADCRAB_RANGE_ATTACK1 = LAST_ASW_ALIEN_JUMPER_SHARED_SCHEDULE,
...
enum 
{
	TASK_HEADCRAB_HOP_ASIDE = LAST_ASW_ALIEN_JUMPER_SHARED_TASK,
...
enum
{
	COND_HEADCRAB_IN_WATER = CASW_Alien_Jumper::NEXT_CONDITION,
...
// Skill settings.
//-----------------------------------------------------------------------------
ConVar	sk_headcrab_health( "sk_headcrab_health", "10", FCVAR_CHEAT );
ConVar	sk_headcrab_fast_health( "sk_headcrab_fast_health", "10", FCVAR_CHEAT );
ConVar	sk_headcrab_poison_health( "sk_headcrab_poison_health", "35", FCVAR_CHEAT );
ConVar	sk_headcrab_melee_dmg( "sk_headcrab_melee_dmg", "5", FCVAR_CHEAT );
ConVar	sk_headcrab_poison_npc_damage( "sk_headcrab_poison_npc_damage", "20", FCVAR_CHEAT );
...
void CBaseHeadcrab::Spawn( void )
{
	...
	if ( m_spawnflags & SF_HEADCRAB_START_HIDDEN )
	{
		...
		// reactivedrop: commented for now, TODO: uncomment
		//SetRenderColorA( 0 );
...
void CBaseHeadcrab::PrescheduleThink( void )
{
	...
	// reactivedrop: commented, TODO: check to uncomment 
...
{
	// reactivedrop: commented, causes link errors, TODO uncomment
...
void CBaseHeadcrab::ClearBurrowPoint( const Vector &origin )
{
	...
	{
		if ( pEntity->m_takedamage != DAMAGE_NO && pEntity->Classify() != CLASS_PLAYER && pEntity->Classify() != CLASS_ASW_MARINE && pEntity->VPhysicsGetObject() )
		{
