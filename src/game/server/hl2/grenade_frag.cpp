...
ConVar sk_plr_dmg_fraggrenade( "sk_plr_dmg_fraggrenade", "125", FCVAR_CHEAT );
ConVar sk_npc_dmg_fraggrenade( "sk_npc_dmg_fraggrenade", "75", FCVAR_CHEAT );
ConVar sk_fraggrenade_radius( "sk_fraggrenade_radius", "250", FCVAR_CHEAT );

#define GRENADE_MODEL "models/Weapons/w_grenade.mdl"

class CGrenadeFrag : public CASW_Grenade_Cluster
{
	DECLARE_CLASS( CGrenadeFrag, CASW_Grenade_Cluster );
...
public:
	...
	void	SetFuseLength( float fSeconds );
...
void CGrenadeFrag::VPhysicsUpdate( IPhysicsObject *pPhysics )
{
	...
	if ( tr.DidHit() )
	{
		...
		CTakeDamageInfo info( this, m_hFirer, pPhysics->GetMass() * vel, GetAbsOrigin(), 0.1f, DMG_CRUSH );
...
void CGrenadeFrag::SetFuseLength( float fSeconds )
{
	SetTimer( fSeconds, fSeconds - FRAG_GRENADE_WARN_TIME );
}

void CGrenadeFrag::SetTimer( float detonateDelay, float warnDelay )
{
	m_fDetonateTime = gpGlobals->curtime + detonateDelay;
...
CASW_Grenade_Cluster *Fraggrenade_Create( const Vector &position, const QAngle &angles, const Vector &velocity, const AngularImpulse &angVelocity, CBaseEntity *pOwner, float timer, bool combineSpawned )
{
	...
	pGrenade->m_hFirer = ToBaseCombatCharacter( pOwner );
