...
void CRD_VGUI_Main_Menu_HoIAF_Leaderboard_Entry::SetFromEntry( const LeaderboardEntry_t &entry, const LeaderboardScoreDetails_Points_t &details )
{
	...
	if ( pFriends )
	{
		...
		V_UTF8ToUnicode( pFriends->GetFriendPersonaName( entry.m_steamIDUser ), wszName, sizeof( wszName ) );
		SetText( wszName );
	}

	CSteamID steamID = entry.m_steamIDUser;
	m_pAvatar->SetAvatarBySteamID( &steamID );
