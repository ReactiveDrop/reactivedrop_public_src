...
void CASW_VGUI_Computer_Download_Docs::PerformLayout()
{
	...
	m_pProgressBarBackdrop->SetPos( 0.5f * w - ( ( BAR_WIDTH + BAR_PADDING * 2 ) * 0.5f ), 0.6f * h );

	m_pProgressBar->SetSize( 0, BAR_HEIGHT );
	m_pProgressBar->SetPos( 0.5f * w - ( BAR_WIDTH * 0.5f ), 0.6f * h + BAR_PADDING );
...
void CASW_VGUI_Computer_Download_Docs::OnThink()
{
	...
	{
		fFraction = m_hHackComputer->GetComputerArea()->GetDownloadProgress();

		m_pBackButton->SetVisible( fFraction >= 1.0f || m_hHackComputer->GetComputerArea()->GetNumMenuOptions() > 1 );
	}

	m_pProgressBar->SetSize( BAR_WIDTH * fFraction, BAR_HEIGHT );
	m_pProgressBar->SetPos( 0.5f * w - ( BAR_WIDTH * 0.5f ), 0.6f * t + BAR_PADDING );
