	...
	m_pReactorDef( "ReactorStatus" )
{
	...
	UTIL_RD_LoadKeyValuesFromFile( m_pReactorDef, g_pFullFileSystem, pHackComputer->GetComputerArea()->GetPlantFileName(), "GAME" );
...
{
	const char *szSuffix = m_bReactorOnline ? "Online" : "Offline";

	m_pReactorOnlineLabel->SetText( m_pReactorDef->GetString( VarArgs( "Label%s", szSuffix ) ) );
	m_pEfficiencyValueLabel->SetText( m_pReactorDef->GetString( VarArgs( "Efficiency%s", szSuffix ) ) );
	m_pEnergyValueLabel->SetText( m_pReactorDef->GetString( VarArgs( "Energy%s", szSuffix ) ) );

	for ( int i = 0; i < 3; i++ )
	{
		m_pCoolantValueLabel[i]->SetText( m_pReactorDef->GetString( VarArgs( "Coolant%d%s", i + 1, szSuffix ) ) );
	}

	for ( int i = 0; i < 4; i++ )
	{
		m_pTemperatureValueLabel[i]->SetText( m_pReactorDef->GetString( VarArgs( "Temperature%d%s", i + 1, szSuffix ) ) );
	}
...
void CASW_VGUI_Computer_Plant::ScrollRawTable()
{
	...
	for ( int i = 9; i >= 1; i-- )
	{
		m_pEnergyBusLabel[i - 1]->GetText( buffer, sizeof( buffer ) );
	...
	if ( m_bReactorOnline )
	{
		V_snprintf( buffer, sizeof( buffer ), "%.2X %.2X %.2X %.2X %.2X %.2X %.2X %.2X %.2X %.2X",
...
bool CASW_VGUI_Computer_Plant::GetReactorOnline()
{
	...
	C_ASW_Computer_Area *pComputerArea = m_pHackComputer->GetComputerArea();
	if ( pComputerArea && pComputerArea->m_iReactorState != -1 )
	{
		return pComputerArea->m_iReactorState != 0;
	}
