	...
	vgui::Panel( parent, name )
{
	...
	SetBgColor( Color( 35, 77, 111, 255 ) );
}
...
void StatsBar::OnThink()
{
	...
	{
		if ( m_bDisplayPercentage )
			Q_snprintf( buffer, sizeof( buffer ), "%f%%%s", current_float, maxbuffer );
		else
			Q_snprintf( buffer, sizeof( buffer ), "%f%s", current_float, maxbuffer );
	}
