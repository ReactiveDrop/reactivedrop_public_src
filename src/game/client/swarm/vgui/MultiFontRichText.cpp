// This file is mostly based on RichText.cpp from Source SDK 2013.
// Original copyright notice: Copyright Valve Corporation, All rights reserved.
...
// TODO(BenLubar): handle different font heights!
...
namespace vgui
{
	...
	//-----------------------------------------------------------------------------
	class MultiFontClickPanel : public Panel
	{
		DECLARE_CLASS_SIMPLE( MultiFontClickPanel, Panel );

	public:
		MultiFontClickPanel( Panel *parent )
		{
			_viewIndex = 0;
			_textIndex = 0;
			SetParent( parent );
	...
	//-----------------------------------------------------------------------------
	class MultiFontRichTextInterior : public Panel
	{
		DECLARE_CLASS_SIMPLE( MultiFontRichTextInterior, Panel );

	public:
		MultiFontRichTextInterior( MultiFontRichText *pParent, const char *pchName ) : BaseClass( pParent, pchName )
		{
	...
	private:
		MultiFontRichText *m_pRichText;
		//	IAppearance *m_pAppearanceScrollbar;    
	};

};	// namespace vgui

DECLARE_BUILD_FACTORY( MultiFontRichText );

//-----------------------------------------------------------------------------
// Purpose: Constructor
//-----------------------------------------------------------------------------
MultiFontRichText::MultiFontRichText( Panel *parent, const char *panelName ) : BaseClass( parent, panelName )
{
	m_bAllTextAlphaIsZero = false;
	m_hFontDefault = INVALID_FONT;
	...
	m_pInterior = new MultiFontRichTextInterior( this, NULL );
...
// Purpose: Destructor
//-----------------------------------------------------------------------------
MultiFontRichText::~MultiFontRichText()
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void MultiFontRichText::SetDrawOffsets( int ofsx, int ofsy )
{
	_drawOffsetX = ofsx;
	_drawOffsetY = ofsy;
}
...
//-----------------------------------------------------------------------------
// Purpose: sets it as drawing text only - used for embedded MultiFontRichText control into other text drawing situations
//-----------------------------------------------------------------------------
void MultiFontRichText::SetDrawTextOnly()
{
	SetDrawOffsets( 0, 0 );
	SetPaintBackgroundEnabled( false );
	//	SetPaintAppearanceEnabled( false );
...
//-----------------------------------------------------------------------------
void MultiFontRichText::ApplySchemeSettings( IScheme *pScheme )
{
	BaseClass::ApplySchemeSettings( pScheme );

	m_hFontDefault = pScheme->GetFont( "Default", IsProportional() );
	m_hFontUnderline = pScheme->GetFont( "DefaultUnderline", IsProportional() );

	FOR_EACH_VEC( m_FormatStream, i )
	{
		if ( m_FormatStream[i].font == INVALID_FONT )
		{
			m_FormatStream[i].font = m_FormatStream[i].textClickable ? m_hFontUnderline : m_hFontDefault;
		}
	}
...
//-----------------------------------------------------------------------------
void MultiFontRichText::SetFgColor( Color color )
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::OnKillFocus()
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::OnSizeChanged( int wide, int tall )
{
...
const wchar_t *MultiFontRichText::ResolveLocalizedTextAndVariables( char const *pchLookup, wchar_t *outbuf, size_t outbufsizeinbytes )
{
	if ( pchLookup[0] == '#' )
	{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::SetText( const char *text )
{
	if ( !text )
	{
		text = "";
...
// Purpose: 
//-----------------------------------------------------------------------------
void MultiFontRichText::SetText( const wchar_t *text )
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::CursorToPixelSpace( int cursorPos, int &cx, int &cy )
{
	...
	for ( int i = GetStartDrawIndex( lineBreakIndexIndex ); i < m_TextStream.Count(); i++ )
	{
		...
		x += surface()->GetCharacterWidth( m_hFontDefault, ch ); // TODO(BenLubar): use correct font from format stream
	}
...
//-----------------------------------------------------------------------------
int MultiFontRichText::PixelToCursorSpace( int cx, int cy )
{
	int fontTall = GetLineHeight();
...
//-----------------------------------------------------------------------------
int MultiFontRichText::DrawString( int iFirst, int iLast, TRenderState &renderState )
{
	//	VPROF( "MultiFontRichText::DrawString" );

		// Calculate the render size
	int fontTall = surface()->GetFontTall( renderState.font );
	...
	for ( int i = iFirst; i <= iLast; i++ )
	{
		...
		surface()->GetKernedCharWidth( renderState.font, ch, chBefore, chAfter, flWide, flabcA );
		if ( ch == L' ' )
			flWide = ceil( flWide );
		charWide += floor( flWide + 0.6 );
#else
		charWide += surface()->GetCharacterWidth( renderState.font, ch );
#endif
	}
	...
	Color textColor = renderState.textColor;
...
//-----------------------------------------------------------------------------
void MultiFontRichText::FinishingURL( int x, int y )
{
	// finishing URL
	if ( _clickableTextPanels.IsValidIndex( _clickableTextIndex ) )
	{
		MultiFontClickPanel *clickPanel = _clickableTextPanels[_clickableTextIndex];
		int px, py;
...
void MultiFontRichText::CalculateFade( TRenderState &renderState )
{
...
void MultiFontRichText::Paint()
{
	...
	surface()->DrawSetTextFont( renderState.font );
	...
	for ( int i = startIndex; i < m_TextStream.Count() && renderState.y < tall; )
	{
		...
		if ( UpdateRenderState( i, renderState ) || i == startIndex )
		{
			...
			if ( renderState.textClickable != _currentTextClickable )
			{
				if ( renderState.textClickable )
				{
					...
					// set up the panel
					MultiFontClickPanel *clickPanel = _clickableTextPanels.IsValidIndex( _clickableTextIndex ) ? _clickableTextPanels[_clickableTextIndex] : NULL;
		...
		if ( iLim <= i )
		{
			if ( m_TextStream[i] == '\t' )
			{
				int dxTabWidth = 8 * surface()->GetCharacterWidth( renderState.font, ' ' );
		...
		{
			renderState.x += DrawString( i, iLim - 1, renderState );
			i = iLim;
		}
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
int MultiFontRichText::GetClickableTextIndexStart( int startIndex )
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::RecalculateDefaultState( int startIndex )
{
	if ( !HasText() )
		return;
...
//-----------------------------------------------------------------------------
bool MultiFontRichText::UpdateRenderState( int textStreamPos, TRenderState &renderState )
{
...
//-----------------------------------------------------------------------------
int MultiFontRichText::FindFormatStreamIndexForTextStreamPos( int textStreamIndex )
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::GenerateRenderStateForTextStreamIndex( int textStreamIndex, TRenderState &renderState )
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::OnThink()
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::PerformLayout()
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::InsertColorChange( Color col )
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::InsertFade( float flSustain, float flLength )
{
	// see if color already exists in text stream
	TFormatStream &prevItem = m_FormatStream[m_FormatStream.Count() - 1];
...
void MultiFontRichText::ResetAllFades( bool bHold, bool bOnlyExpired, float flNewSustain )
{
	m_bResetFades = bHold;
...
//-----------------------------------------------------------------------------
void MultiFontRichText::InsertFontChange( HFont font )
{
	// see if font already exists in text stream
	TFormatStream &prevItem = m_FormatStream[m_FormatStream.Count() - 1];
	if ( prevItem.font == font )
	{
		// inserting same font into stream, just ignore
	}
	else if ( prevItem.textStreamIndex == m_TextStream.Count() )
	{
		// this item is in the same place; update values
...
//-----------------------------------------------------------------------------
void MultiFontRichText::InsertIndentChange( int pixelsIndent )
{
	if ( pixelsIndent < 0 )
	{
		pixelsIndent = 0;
...
//-----------------------------------------------------------------------------
void MultiFontRichText::InsertClickableTextStart( const char *pchClickAction )
{
	// see if indent change already exists in text stream
	TFormatStream &prevItem = m_FormatStream[m_FormatStream.Count() - 1];
	...
	if ( prevItem.textStreamIndex == m_TextStream.Count() )
	{
		...
		prevItem.font = m_hFontUnderline;
	...
	else
	{
		...
		pFormatStream->font = m_hFontUnderline;
		pFormatStream->m_sClickableTextAction = pchClickAction;
	}
...
//-----------------------------------------------------------------------------
void MultiFontRichText::InsertClickableTextEnd()
{
	// see if indent change already exists in text stream
	TFormatStream &prevItem = m_FormatStream[m_FormatStream.Count() - 1];
	...
	else
	{
		...
		streamItem.textStreamIndex = m_TextStream.Count();
		streamItem.font = m_hFontDefault;
		m_FormatStream.AddToTail( streamItem );
	}
}
...
//-----------------------------------------------------------------------------
void MultiFontRichText::AddAnotherLine( int &cx, int &cy )
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::RecalculateLineBreaks()
{
	if ( !m_bRecalcLineBreaks )
		return;
	...
	for ( int i = startChar; i < m_TextStream.Count(); ++i )
	{
		...
		if ( UpdateRenderState( i, renderState ) )
		{
			...
			if ( renderState.textClickable != _currentTextClickable )
			{
				if ( renderState.textClickable )
				{
					...
					{
						_clickableTextPanels.AddToTail( new MultiFontClickPanel( this ) );
					}

					MultiFontClickPanel *clickPanel = _clickableTextPanels[clickableTextNum++];
		...
		if ( !bIsWSpace && ch != L'\t' && ch != L'\n' && ch != L'\r' )
		{
			if ( !hasWord )
			{
				...
				fontWordStart = renderState.font;
			}
		...
		float flabcA;
		surface()->GetKernedCharWidth( renderState.font, ch, wchBefore, wchAfter, w, flabcA );
#else
		w = surface()->GetCharacterWidth( renderState.font, ch );
#endif
		flLineWidthSoFar += w;
...
//-----------------------------------------------------------------------------
void MultiFontRichText::LayoutVerticalScrollBarSlider()
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::SetVerticalScrollbar( bool state )
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::CreateEditMenu()
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::OnMouseFocusTicked()
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::OnCursorEntered()
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::OnCursorExited()
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::OnCursorMoved( int x, int y )
{
	if ( _mouseSelection )
	{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::OnMousePressed( MouseCode code )
{
	if ( code == MOUSE_LEFT )
	{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::OnMouseReleased( MouseCode code )
{
	_mouseSelection = false;
...
//-----------------------------------------------------------------------------
void MultiFontRichText::OnMouseDoublePressed( MouseCode code )
{
	if ( !m_bInteractive )
		return;
...
//-----------------------------------------------------------------------------
void MultiFontRichText::OnMouseCaptureLost()
{
	_mouseSelection = false;
}
...
//-----------------------------------------------------------------------------
void MultiFontRichText::OnKeyCodeTyped( KeyCode code )
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::OnMouseWheeled( int delta )
{
	MoveScrollBar( delta );
}
...
//-----------------------------------------------------------------------------
void MultiFontRichText::MoveScrollBar( int delta )
{
...
// Input  : delta - amount to move scrollbar up
//-----------------------------------------------------------------------------
void MultiFontRichText::MoveScrollBarDirect( int delta )
{
	if ( _vertScrollBar->IsVisible() )
	{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::SetMaximumCharCount( int maxChars )
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::GotoWordRight()
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::GotoWordLeft()
{
	if ( _cursorPos < 1 )
		return;
...
//-----------------------------------------------------------------------------
void MultiFontRichText::GotoTextStart()
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::GotoTextEnd()
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::TruncateTextStream()
{
	if ( _maxCharCount < 1 )
		return;
...
//-----------------------------------------------------------------------------
void MultiFontRichText::InsertChar( wchar_t wch )
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::InsertString( const char *text )
{
	if ( text[0] == '#' )
	{
		wchar_t unicode[1024];
...
//-----------------------------------------------------------------------------
void MultiFontRichText::InsertString( const wchar_t *wszText )
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::SelectNone()
{
...
//-----------------------------------------------------------------------------
bool MultiFontRichText::GetSelectedRange( int &cx0, int &cx1 )
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::OpenEditMenu()
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::CutSelected()
{
	CopySelected();
...
//-----------------------------------------------------------------------------
void MultiFontRichText::CopySelected()
{
	int x0, x1;
...
//-----------------------------------------------------------------------------
int MultiFontRichText::GetStartDrawIndex( int &lineBreakIndexIndex )
{
	int startIndex = 0;
...
//-----------------------------------------------------------------------------
void MultiFontRichText::GetText( int offset, wchar_t *buf, int bufLenInBytes )
{
	if ( !buf )
		return;
...
//-----------------------------------------------------------------------------
void MultiFontRichText::GetText( int offset, char *pch, int bufLenInBytes )
{
	wchar_t rgwchT[4096];
...
//-----------------------------------------------------------------------------
void MultiFontRichText::SetFont( HFont font )
{
	// Replace default format font if 
	// the stream is empty and the font is the default ( or the previous font )
	if ( m_FormatStream.Count() == 1 &&
		( m_FormatStream[0].font == m_hFontDefault ) )
	{
		m_FormatStream[0].font = font;
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
bool MultiFontRichText::RequestInfo( KeyValues *outputData )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void MultiFontRichText::OnSetText( const wchar_t *text )
{
	SetText( text );
}
...
//-----------------------------------------------------------------------------
void MultiFontRichText::OnClickPanel( int index )
{
	wchar_t wBuf[512];
	int outIndex = 0;
...
// Purpose: 
//-----------------------------------------------------------------------------
void MultiFontRichText::ApplySettings( KeyValues *inResourceData )
{
	...
	else
	{
		...
		if ( textfilename )
		{
			...
			if ( !f )
			{
				Warning( "MultiFontRichText: textfile parameter '%s' not found.\n", textfilename );
				return;
			}
...
// Purpose: 
//-----------------------------------------------------------------------------
void MultiFontRichText::GetSettings( KeyValues *outResourceData )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
const char *MultiFontRichText::GetDescription()
{
...
//-----------------------------------------------------------------------------
int MultiFontRichText::GetNumLines()
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::SelectAllText()
{
	_cursorPos = 0;
	_select[0] = 0;
...
// Purpose: Select all the text.
//-----------------------------------------------------------------------------
void MultiFontRichText::SelectNoText()
{
	_select[0] = 0;
	_select[1] = 0;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void MultiFontRichText::OnSetFocus()
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::InvalidateLineBreakStream()
{
	// clear the buffer
	m_LineBreaks.RemoveAll();
...
//-----------------------------------------------------------------------------
void MultiFontRichText::InsertPossibleURLString( const char *text, Color URLTextColor, Color normalTextColor )
{
...
//-----------------------------------------------------------------------------
int MultiFontRichText::ParseTextStringForUrls( const char *text, int startPos, char *pchURLText, int cchURLText, char *pchURL, int cchURL, bool &clickable )
{
...
// default.
//-----------------------------------------------------------------------------
void MultiFontRichText::OnTextClicked( const wchar_t *wszText )
{
...
// Purpose:
//-----------------------------------------------------------------------------
void MultiFontRichText::SetURLClickedHandler( Panel *pPanelToHandleClickMsg )
{
...
//-----------------------------------------------------------------------------
bool MultiFontRichText::IsScrollbarVisible()
{
	return _vertScrollBar->IsVisible();
}

void MultiFontRichText::SetUnderlineFont( HFont font )
{
	m_hFontUnderline = font;
}

bool MultiFontRichText::IsAllTextAlphaZero() const
{
	return m_bAllTextAlphaIsZero;
}

bool MultiFontRichText::HasText() const
{
...
//-----------------------------------------------------------------------------
int MultiFontRichText::GetLineHeight()
{
...
//-----------------------------------------------------------------------------
void MultiFontRichText::Validate( CValidator &validator, char *pchName )
{
	validator.Push( "vgui::MultiFontRichText", this, pchName );
	...
	BaseClass::Validate( validator, "vgui::MultiFontRichText" );
