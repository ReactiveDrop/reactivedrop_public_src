...
void CASW_VGUI_Computer_Stocks::SetStockLabels()
{
	...
	const char *pszStocksFile = pArea->m_StocksSeed.Get();
	int iSeed = atoi( pszStocksFile );
	if ( !g_pFullFileSystem->FileExists( VarArgs( "resource/stocks/%s", pszStocksFile ), "GAME" ) )
	{
		pszStocksFile = "default.txt";
	}

	KeyValues::AutoDelete pKV( "Stocks" );
	UTIL_RD_LoadKeyValuesFromFile( pKV, g_pFullFileSystem, VarArgs( "resource/stocks/%s", pszStocksFile ), "GAME" );
	...
	{
		m_pSymbol[i]->SetText( pKV->GetString( VarArgs( "Symbol%d", i ) ) );
		m_pCorp[i]->SetText( pKV->GetString( VarArgs( "Name%d", i ) ) );

		float fCurrent = pKV->GetFloat( VarArgs( "Value%d", i ) );
		float fChange = pKV->GetFloat( VarArgs( "Change%d", i ) );
		int iVolume = pKV->GetInt( VarArgs( "Volume%d", i ) );
		...
		if ( iSeed )
		{
			...
			fCurrent += NumberStream.RandomInt( -500, 500 ) / 100.0f;
			fChange = NumberStream.RandomInt( -400, 400 ) / 100.0f;
			iVolume = NumberStream.RandomInt( 25000000, 120000000 );
		}

		char buffer[64];

		V_snprintf( buffer, sizeof( buffer ), "%.2f", fCurrent );
		m_pValue[i]->SetText( buffer );

		V_snprintf( buffer, sizeof( buffer ), "%+.2f", fChange );
		m_pChange[i]->SetText( buffer );

		V_snprintf( buffer, sizeof( buffer ), "%d", iVolume );
