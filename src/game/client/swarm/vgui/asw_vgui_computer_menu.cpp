	...
	m_hHackComputer( pHackComputer )
{
	for ( int i = 0; i < ASW_COMPUTER_MAX_MENU_ITEMS; i++ )
	{
		...
		m_pMenuLabel[i]->SetCommand( new KeyValues( "Command", "command", VarArgs( "Option%d", i ) ) );
		m_pMenuLabel[i]->SetCancelCommand( new KeyValues( "Command", "command", "Cancel" ) );
...
void CASW_VGUI_Computer_Menu::LayoutMenuOptions()
{
	...
	m_iOptions = pArea->GetNumMenuOptions();
...
void CASW_VGUI_Computer_Menu::SetIcons()
{
	...
	if ( pArea->m_DownloadObjectiveName.Get()[0] != 0 && pArea->GetDownloadProgress() < 1.0f )
		SetIcon( icon++, "#asw_SynTekDocs", "swarm/Computer/IconFiles", pArea->m_iLockedScreens & ( 1 << COMPUTER_LOCKED_DOWNLOAD ) );
	if ( pArea->m_hSecurityCam1.Get() )
		SetIcon( icon++, "#asw_SynTekSecurityCam1", "swarm/Computer/IconCamera", pArea->m_iLockedScreens & ( 1 << COMPUTER_LOCKED_SECURITY_CAM_1 ) );
	if ( pArea->m_hSecurityCam2.Get() )
		SetIcon( icon++, "#asw_SynTekSecurityCam2", "swarm/Computer/IconCamera", pArea->m_iLockedScreens & ( 1 << COMPUTER_LOCKED_SECURITY_CAM_2 ) );
	if ( pArea->m_hSecurityCam3.Get() )
		SetIcon( icon++, "#asw_SynTekSecurityCam3", "swarm/Computer/IconCamera", pArea->m_iLockedScreens & ( 1 << COMPUTER_LOCKED_SECURITY_CAM_3 ) );
	if ( pArea->m_hTurret1.Get() )
		SetIcon( icon++, "#asw_SynTekTurret1", "swarm/Computer/IconTurret", pArea->m_iLockedScreens & ( 1 << COMPUTER_LOCKED_TURRET_1 ) );
	if ( pArea->m_hTurret2.Get() )
		SetIcon( icon++, "#asw_SynTekTurret2", "swarm/Computer/IconTurret", pArea->m_iLockedScreens & ( 1 << COMPUTER_LOCKED_TURRET_2 ) );
	if ( pArea->m_hTurret3.Get() )
		SetIcon( icon++, "#asw_SynTekTurret3", "swarm/Computer/IconTurret", pArea->m_iLockedScreens & ( 1 << COMPUTER_LOCKED_TURRET_3 ) );
	if ( pArea->m_MailFile.Get()[0] != 0 )
		SetIcon( icon++, "#asw_SynTekMail", "swarm/Computer/IconMail", pArea->m_iLockedScreens & ( 1 << COMPUTER_LOCKED_MAIL ) );
	if ( pArea->m_NewsFile.Get()[0] != 0 )
		SetIcon( icon++, "#asw_SynTekNews", "swarm/Computer/IconNewsLCD", pArea->m_iLockedScreens & ( 1 << COMPUTER_LOCKED_NEWS ) );
	if ( pArea->m_StocksSeed.Get()[0] != 0 )
		SetIcon( icon++, "#asw_SynTekStocks", "swarm/Computer/IconStocks", pArea->m_iLockedScreens & ( 1 << COMPUTER_LOCKED_STOCKS ) );
	if ( pArea->m_WeatherSeed.Get()[0] != 0 )
		SetIcon( icon++, "#asw_SynTekAtmosphere", "swarm/Computer/IconWeather", pArea->m_iLockedScreens & ( 1 << COMPUTER_LOCKED_WEATHER ) );
	if ( pArea->m_PlantFile.Get()[0] != 0 )
		SetIcon( icon++, "#asw_SynTekPlantStatus", "swarm/Computer/IconCog", pArea->m_iLockedScreens & ( 1 << COMPUTER_LOCKED_PLANT ) );
	if ( pArea->m_hCustomScreen1.Get() )
		SetIcon( icon++, pArea->m_hCustomScreen1->m_szLabel, VarArgs( "../%s", pArea->m_hCustomScreen1->m_szIconName.Get() ), pArea->m_iLockedScreens & ( 1 << COMPUTER_LOCKED_CUSTOM_SCREEN_1 ) );
	if ( pArea->m_hCustomScreen2.Get() )
		SetIcon( icon++, pArea->m_hCustomScreen2->m_szLabel, VarArgs( "../%s", pArea->m_hCustomScreen2->m_szIconName.Get() ), pArea->m_iLockedScreens & ( 1 << COMPUTER_LOCKED_CUSTOM_SCREEN_2 ) );
	if ( pArea->m_hCustomScreen3.Get() )
		SetIcon( icon++, pArea->m_hCustomScreen3->m_szLabel, VarArgs( "../%s", pArea->m_hCustomScreen3->m_szIconName.Get() ), pArea->m_iLockedScreens & ( 1 << COMPUTER_LOCKED_CUSTOM_SCREEN_3 ) );
	if ( pArea->m_hCustomScreen4.Get() )
		SetIcon( icon++, pArea->m_hCustomScreen4->m_szLabel, VarArgs( "../%s", pArea->m_hCustomScreen4->m_szIconName.Get() ), pArea->m_iLockedScreens & ( 1 << COMPUTER_LOCKED_CUSTOM_SCREEN_4 ) );
	if ( pArea->m_hCustomScreen5.Get() )
		SetIcon( icon++, pArea->m_hCustomScreen5->m_szLabel, VarArgs( "../%s", pArea->m_hCustomScreen5->m_szIconName.Get() ), pArea->m_iLockedScreens & ( 1 << COMPUTER_LOCKED_CUSTOM_SCREEN_5 ) );
	if ( pArea->m_hCustomScreen6.Get() )
		SetIcon( icon++, pArea->m_hCustomScreen6->m_szLabel, VarArgs( "../%s", pArea->m_hCustomScreen6->m_szIconName.Get() ), pArea->m_iLockedScreens & ( 1 << COMPUTER_LOCKED_CUSTOM_SCREEN_6 ) );
}

void CASW_VGUI_Computer_Menu::SetIcon( int icon, const char *szLabel, const char *szIcon, bool bLocked )
{
...
void CASW_VGUI_Computer_Menu::ApplySchemeSettings( vgui::IScheme *pScheme )
{
	...
	for ( int i = 0; i < ASW_COMPUTER_MAX_MENU_ITEMS; i++ )
	{
		...
		m_pMenuLabel[i]->SetBgColor( blue );
	...
	m_pAccessDeniedLabel->SetFgColor( red );
	m_pInsufficientRightsLabel->SetFont( DefaultFont );
	m_pInsufficientRightsLabel->SetFgColor( white );
...
void CASW_VGUI_Computer_Menu::OnThink()
{
	...
	if ( m_bFadingCurrentPage )
	{
		if ( m_hCurrentPage.Get() == NULL || m_hCurrentPage->GetAlpha() <= 0 )
		{
			...
			bool bAutodownload = ( m_iAutodownload != -1 ) && m_hHackComputer && m_hHackComputer->GetComputerArea() && !m_hHackComputer->GetComputerArea()->IsLocked();
			if ( bAutodownload )
			{
...
void CASW_VGUI_Computer_Menu::LaunchHackOption( int iOption )
{
	...
	if ( iOption == ASW_HACK_OPTION_OVERRIDE )
	{
		...
		{
			CASW_VGUI_Computer_Tumbler_Hack *pPage = new CASW_VGUI_Computer_Tumbler_Hack( this, "ComputerTumblerHack", m_hHackComputer );
			pPage->SetPos( 0, 0 );
			m_hCurrentPage = pPage;
		}
	...
	if ( pArea->m_DownloadObjectiveName.Get()[0] != 0 && pArea->GetDownloadProgress() < 1.0f )
	{
		...
		{
			// activate this one
			CASW_VGUI_Computer_Download_Docs *pPage = new CASW_VGUI_Computer_Download_Docs( this, "ComputerDocs", m_hHackComputer );
			pPage->ASWInit();
			pPage->SetPos( 0, 0 );
			m_hCurrentPage = pPage;
			return;
	...
	if ( pArea->m_hSecurityCam1.Get() )
	{
		if ( iOption == icon )
		{
			...
			CASW_VGUI_Computer_Camera *pCam = new CASW_VGUI_Computer_Camera( this, "ComputerCamera", m_hHackComputer, pArea->m_SecurityCamLabel1 );
			pCam->ASWInit();
			pCam->SetPos( 0, 0 );
			m_hCurrentPage = pCam;
			return;
	...
	if ( pArea->m_hSecurityCam2.Get() )
	{
		if ( iOption == icon )
		{
			CASW_VGUI_Computer_Camera *pCam = new CASW_VGUI_Computer_Camera( this, "ComputerCamera", m_hHackComputer, pArea->m_SecurityCamLabel2 );
			pCam->ASWInit();
			pCam->SetPos( 0, 0 );
			m_hCurrentPage = pCam;
			return;
	...
	if ( pArea->m_hSecurityCam3.Get() )
	{
		if ( iOption == icon )
		{
			CASW_VGUI_Computer_Camera *pCam = new CASW_VGUI_Computer_Camera( this, "ComputerCamera", m_hHackComputer, pArea->m_SecurityCamLabel3 );
			pCam->ASWInit();
			pCam->SetPos( 0, 0 );
			m_hCurrentPage = pCam;
			return;
	...
	if ( pArea->m_MailFile.Get()[0] != 0 )
	{
		...
		{
			// activate this one
			CASW_VGUI_Computer_Mail *pPage = new CASW_VGUI_Computer_Mail( this, "ComputerMail", m_hHackComputer );
			pPage->ASWInit();
			pPage->SetPos( 0, 0 );
			m_hCurrentPage = pPage;
			return;
	...
	if ( pArea->m_NewsFile.Get()[0] != 0 )
	{
		...
		{
			// activate this one
			CASW_VGUI_Computer_News *pPage = new CASW_VGUI_Computer_News( this, "ComputerNews", m_hHackComputer );
			pPage->ASWInit();
			pPage->SetPos( 0, 0 );
			m_hCurrentPage = pPage;
			return;
	...
	if ( pArea->m_StocksSeed.Get()[0] != 0 )
	{
		...
		{
			// activate this one			
			CASW_VGUI_Computer_Stocks *pPage = new CASW_VGUI_Computer_Stocks( this, "ComputerStocks", m_hHackComputer );
			pPage->ASWInit();
			pPage->SetPos( 0, 0 );
			m_hCurrentPage = pPage;
			return;
	...
	if ( pArea->m_WeatherSeed.Get()[0] != 0 )
	{
		if ( iOption == icon )
		{
			CASW_VGUI_Computer_Weather *pPage = new CASW_VGUI_Computer_Weather( this, "ComputerWeather", m_hHackComputer );
			pPage->ASWInit();
			pPage->SetPos( 0, 0 );
			m_hCurrentPage = pPage;
			return;
	...
	if ( pArea->m_PlantFile.Get()[0] != 0 )
	{
		...
		{
			// activate this one
			CASW_VGUI_Computer_Plant *pPage = new CASW_VGUI_Computer_Plant( this, "ComputerPlant", m_hHackComputer );
			pPage->ASWInit();
			pPage->SetPos( 0, 0 );
			m_hCurrentPage = pPage;
			return;
	...
	if ( pArea->m_hCustomScreen1.Get() )
	{
		...
		{
			// activate this one
			CASW_VGUI_Computer_Custom *pPage = new CASW_VGUI_Computer_Custom( this, "ComputerCustom", m_hHackComputer, pArea->m_hCustomScreen1 );
			pPage->ASWInit();
			pPage->SetPos( 0, 0 );
			m_hCurrentPage = pPage;
			return;
	...
	if ( pArea->m_hCustomScreen2.Get() )
	{
		...
		{
			// activate this one
			CASW_VGUI_Computer_Custom *pPage = new CASW_VGUI_Computer_Custom( this, "ComputerCustom", m_hHackComputer, pArea->m_hCustomScreen2 );
			pPage->ASWInit();
			pPage->SetPos( 0, 0 );
			m_hCurrentPage = pPage;
			return;
	...
	if ( pArea->m_hCustomScreen3.Get() )
	{
		...
		{
			// activate this one
			CASW_VGUI_Computer_Custom *pPage = new CASW_VGUI_Computer_Custom( this, "ComputerCustom", m_hHackComputer, pArea->m_hCustomScreen3 );
			pPage->ASWInit();
			pPage->SetPos( 0, 0 );
			m_hCurrentPage = pPage;
			return;
	...
	if ( pArea->m_hCustomScreen4.Get() )
	{
		...
		{
			// activate this one
			CASW_VGUI_Computer_Custom *pPage = new CASW_VGUI_Computer_Custom( this, "ComputerCustom", m_hHackComputer, pArea->m_hCustomScreen4 );
			pPage->ASWInit();
			pPage->SetPos( 0, 0 );
			m_hCurrentPage = pPage;
			return;
	...
	if ( pArea->m_hCustomScreen5.Get() )
	{
		...
		{
			// activate this one
			CASW_VGUI_Computer_Custom *pPage = new CASW_VGUI_Computer_Custom( this, "ComputerCustom", m_hHackComputer, pArea->m_hCustomScreen5 );
			pPage->ASWInit();
			pPage->SetPos( 0, 0 );
			m_hCurrentPage = pPage;
			return;
	...
	if ( pArea->m_hCustomScreen6.Get() )
	{
		...
		{
			// activate this one
			CASW_VGUI_Computer_Custom *pPage = new CASW_VGUI_Computer_Custom( this, "ComputerCustom", m_hHackComputer, pArea->m_hCustomScreen6 );
			pPage->ASWInit();
			pPage->SetPos( 0, 0 );
			m_hCurrentPage = pPage;
			return;
	...
	Assert( !"ran out of computer options" );
}
...
bool CASW_VGUI_Computer_Menu::IsHacking()
{
	return ( dynamic_cast< CASW_VGUI_Computer_Tumbler_Hack * >( m_hCurrentPage.Get() ) != NULL ) ||
		( dynamic_cast< CASW_VGUI_Computer_Custom_Hack * >( m_hCurrentPage.Get() ) != NULL );
}
