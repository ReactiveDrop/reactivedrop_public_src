...
CASW_Mission_Chooser_Entry::CASW_Mission_Chooser_Entry( vgui::Panel *pParent, const char *pElementName, ASW_CHOOSER_TYPE iChooserType, ASW_HOST_TYPE iHostType ) :
	vgui::Panel( pParent, pElementName )
{
	...
	if ( iChooserType == ASW_CHOOSER_TYPE::SAVED_CAMPAIGN && iHostType != ASW_HOST_TYPE::CALLVOTE )
	{
...
void CASW_Mission_Chooser_Entry::OnThink()
{
	...
	else if ( m_bVoteDisabled && m_ChooserType == ASW_CHOOSER_TYPE::CAMPAIGN && m_nFileID != k_PublishedFileIdInvalid )
	{
...
void CASW_Mission_Chooser_Entry::SetDetails( const char *szMapName, ASW_CHOOSER_TYPE nChooserType )
{
	m_ChooserType = nChooserType;
	SetDetails( szMapName );
}

void CASW_Mission_Chooser_Entry::SetDetails( const char *szMapName )
{
	if ( m_ChooserType == ASW_CHOOSER_TYPE::SINGLE_MISSION )
	{
		if ( V_stricmp( szMapName, m_szMapName ) )
		{
			m_bWaitingForWorkshop = false;
			m_nFileID = k_PublishedFileIdInvalid;

			V_snprintf( m_szMapName, sizeof( m_szMapName ), "%s", szMapName );
