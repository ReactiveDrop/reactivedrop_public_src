...
void CASW_VGUI_Computer_Weather::SetWeatherLabels()
{
	...
	const char *pszWeatherFile = pArea->m_WeatherSeed.Get();
	int iSeed = atoi( pszWeatherFile );
	if ( !g_pFullFileSystem->FileExists( VarArgs( "resource/weather/%s", pszWeatherFile ), "GAME" ) )
	{
		pszWeatherFile = "default.txt";
	}

	KeyValues::AutoDelete pKV( "Weather" );
	UTIL_RD_LoadKeyValuesFromFile( pKV, g_pFullFileSystem, VarArgs( "resource/weather/%s", pszWeatherFile ), "GAME" );

	const char *szTemperature = pKV->GetString( "TemperatureFormat", "#asw_weather_temp_r" );
	const char *szWind = pKV->GetString( "WindFormat", "#asw_weather_wind_r" );
	const char *szPressure = pKV->GetString( "PressureFormat", "#asw_weather_pressure_r" );
	const char *szHumidity = pKV->GetString( "HumidityFormat", "#asw_weather_humidity_r" );
	const char *szClouds = pKV->GetString( "CloudsFormat", "#asw_weather_broken_cloud_r" );
	const char *szVisibility = pKV->GetString( "VisibilityFormat", "#asw_weather_vis_r" );
	const char *szWeather = pKV->GetString( "Weather", "#asw_weather_weather_light" );
	const char *szWarning = pKV->GetString( "Warning", "#asw_weather_warning" );
	float fTemperature = pKV->GetFloat( "Temperature" );
	float fWind = pKV->GetFloat( "Wind" );
	int iPressure = pKV->GetInt( "Pressure" );
	float fHumidity = pKV->GetFloat( "Humidity" );
	float fVisibility = pKV->GetFloat( "Visibility" );
	int iClouds = pKV->GetInt( "Clouds" );

	if ( iSeed )
	{
		CUniformRandomStream NumberStream;
		NumberStream.SetSeed( iSeed );

		fWind = NumberStream.RandomInt( 0, 200 ) / 10.0f;
		iPressure = NumberStream.RandomInt( 1000, 1030 );
		fHumidity = NumberStream.RandomInt( 6000, 9000 ) / 100.0f;
		fVisibility = NumberStream.RandomInt( 10, 90 ) / 10.0f;
		szWeather = ( NumberStream.RandomInt( 0, 10 ) <= 4 ) ? "#asw_weather_weather_light" : "#asw_weather_weather_moderate";
		szClouds = ( NumberStream.RandomInt( 0, 10 ) <= 5 ) ? "#asw_weather_few_cloud_r" : "#asw_weather_broken_cloud_r";
		iClouds = NumberStream.RandomInt( 450, 2000 );
		fTemperature = -6 - ( NumberStream.RandomInt( 0, 140 ) / 10.0f ) + 273.15f;
	}

	SetFloatLabel( m_pWeatherValue[0], szTemperature, fTemperature );
	SetFloatLabel( m_pWeatherValue[1], szWind, fWind );
	SetIntLabel( m_pWeatherValue[2], szPressure, iPressure );
	SetFloatLabel( m_pWeatherValue[3], szHumidity, fHumidity );
	SetIntLabel( m_pWeatherValue[4], szClouds, iClouds );
	SetFloatLabel( m_pWeatherValue[5], szVisibility, fVisibility );
	m_pWeatherValue[6]->SetText( szWeather );
	m_pWarningLabel->SetText( szWarning );
}
