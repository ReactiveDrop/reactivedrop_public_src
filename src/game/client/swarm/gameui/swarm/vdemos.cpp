...
static void RenameRecordingCallback()
{
	...
	{//this seems to be a signature string, the signature string is at least 2592 bytes long
		...
		auto signHexStr = std::string(szNewBase);
		LBDataUnion lbData = {};
		if (SignHexStringToLBData(SteamUser()->GetSteamID(), signHexStr, lbData))
		{
			auto lbDetailsVec = PrepareVerifiedLBDetails(lbData, signHexStr);
			//TODO:
			//dont know if this is right, so currently it wont do anything
			//SteamAPICall_t hAPICall = SteamUserStats()->UploadLeaderboardScore(lbData.data.hLeaderboard, k_ELeaderboardUploadScoreMethodForceUpdate, lbData.data.score, lbDetailsVec.data(), lbDetailsVec.size());
		}
	}
	else
