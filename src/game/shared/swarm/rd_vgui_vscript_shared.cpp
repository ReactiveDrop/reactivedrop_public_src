...
C_BasePlayer *CRD_VGui_VScript::GetPredictionOwner()
{
	...
	if ( m_iDataInt.Get( RD_VGUI_VSCRIPT_LOCAL_MENU_MODE_SLOT ) == RD_VGUI_VSCRIPT_LOCAL_MENU_MODE_SENTINEL )
	{
		C_BasePlayer *pDataPlayer = ToBasePlayer( m_hDataEntity.Get() );
		const bool bLocalMenu = pDataPlayer &&
			pDataPlayer == C_BasePlayer::GetLocalPlayer() &&
			m_iDataInt.Get( 0 ) != 0;
