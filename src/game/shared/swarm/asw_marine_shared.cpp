...
ConVar rd_shieldbug_pierce_wall( "rd_shieldbug_pierce_wall", "1", FCVAR_REPLICATED | FCVAR_CHEAT, "Only allow piercing Shieldbug's shield (asw_shieldbug_pierce) when the bullet can pierce a wall." );
		...
		if (bPierce)
		{
			...
			if ( pNPC || pAlien || pStatue )
			{
				if ( tr.m_pEnt->Classify() == CLASS_ASW_SHIELDBUG )		// don't let bullets pass through shieldbugs
				{
					...
					if ( bShieldbugBlocked
							&& rd_shieldbug_pierce.GetInt() > 0
							&& iMaxPenetrate >= rd_shieldbug_pierce.GetInt()
							&& ( !rd_shieldbug_pierce_wall.GetBool() || fPenetrateChance > 1.0f )
							)
					{
						bPierce = true;
