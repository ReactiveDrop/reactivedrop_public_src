...
void CScratchPadGraph::UpdateTicksAndStuff( float flTime, float flValue )
{
	...
	if ( flValue > m_flHighestValue )
	{
		...
		{
			float flLabelTime = m_flTimeOrigin + m_nTimeLabelsDrawn * m_flTimeLabelEveryNSeconds;

			m_pPad->DrawLine(
				CSPVert((const Vector&) GetSamplePosition( flLabelTime, m_flHighestValue )),
				CSPVert((const Vector&) GetSamplePosition( flLabelTime, flValue ) )	
				);
		}
	...
	while ( m_nTimeLabelsDrawn < iHighestTextLabel )
	{
		...
		params.m_vPos = GetSamplePosition( flLabelTime, m_flValueOrigin-5 );
		params.m_bTwoSided = true;
		
		char str[512];
		Q_snprintf( str, sizeof( str ), "time: %.2f", flLabelTime );
		m_pPad->DrawText( str, params );


		// Now draw the vertical line for the value..
		m_pPad->DrawLine(
			CSPVert(  (const Vector&)GetSamplePosition( flLabelTime, m_flValueOrigin ) ),
			CSPVert( (const Vector&)GetSamplePosition( flLabelTime, m_flHighestValue ) )
			);
